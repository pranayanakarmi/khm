...
const form = useForm({ title: props.program.title, slug: props.program.slug, category: props.program.category || '', description: props.program.description || '', duration: props.program.duration || '', intake: props.program.intake || '', eligibility: props.program.eligibility || '', is_active: props.program.is_active, sort_order: props.program.sort_order, image: null, _method: 'PUT' });
function submit() { form.post(`/admin/programs/${props.program.id}`, { forceFormData: true }); }
    ...
    <AdminLayout>
        ...
        <div class="max-w-3xl">
            <div class="flex items-center gap-3 mb-6">
                ...
                <h2 class="text-xl font-bold">Edit Program</h2>
            </div>
            <form @submit.prevent="submit" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div>
                    ...
                    <div v-if="program.image" class="mb-2"><img :src="`/storage/${program.image}`" class="h-24 rounded-lg object-cover"/></div>
                ...
                <div class="flex gap-3">
                    <button type="submit" :disabled="form.processing" class="bg-blue-800 text-white px-6 py-2 rounded-lg text-sm hover:bg-blue-900 disabled:opacity-50">
                        {{ form.processing ? 'Saving...' : 'Update Program' }}</button>
