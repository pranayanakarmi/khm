...
const form = useForm({ title: '', slug: '', type: 'event', description: '', location: '', organizer: '', start_date: '', end_date: '', is_published: false, image: null });
function submit() { form.post('/admin/events', { forceFormData: true }); }
const types = ['event', 'training', 'workshop', 'seminar', 'conference'];
</script>
<template>
    <AdminLayout><template #title>Create Event</template>
        <div class="max-w-3xl">
            ...
            <form @submit.prevent="submit" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Start Date *</label>
                        <input v-model="form.start_date" type="datetime-local" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                        <p v-if="form.errors.start_date" class="text-red-600 text-xs mt-1">{{ form.errors.start_date }}</p></div>
                ...
                <div class="flex gap-3">
                    <button type="submit" :disabled="form.processing" class="bg-blue-800 text-white px-6 py-2 rounded-lg text-sm hover:bg-blue-900 disabled:opacity-50">
                        {{ form.processing ? 'Saving...' : 'Create Event' }}</button>
