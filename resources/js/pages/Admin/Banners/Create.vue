...
const form = useForm({ title: '', subtitle: '', link: '', is_active: true, sort_order: 0, image: null });
function submit() { form.post('/admin/banners', { forceFormData: true }); }
</script>
<template>
    <AdminLayout>
        <template #title>Create Banner</template>
        <div class="max-w-2xl">
            <div class="flex items-center gap-3 mb-6">
                <Link href="/admin/banners" class="text-gray-500 hover:text-gray-700">← Banners</Link>
                <h2 class="text-xl font-bold text-gray-900">Create Banner</h2>
            </div>
            <form @submit.prevent="submit" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div class="flex gap-3">
                    <button type="submit" :disabled="form.processing" class="bg-blue-800 text-white px-6 py-2 rounded-lg text-sm hover:bg-blue-900 disabled:opacity-50">
                        {{ form.processing ? 'Saving...' : 'Create Banner' }}
                    </button>
