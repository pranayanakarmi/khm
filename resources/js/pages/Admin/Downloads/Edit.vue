...
const props = defineProps({ download: Object });
const form = useForm({ title: props.download.title, category: props.download.category, description: props.download.description || '', is_active: props.download.is_active, sort_order: props.download.sort_order, file: null, _method: 'PUT' });
function submit() { form.post(`/admin/downloads/${props.download.id}`, { forceFormData: true }); }
const cats = ['general', 'academic', 'administrative', 'forms'];
</script>
<template>
    <AdminLayout><template #title>Edit Download</template>
        <div class="max-w-2xl">
            <div class="flex items-center gap-3 mb-6">
                <Link href="/admin/downloads" class="text-gray-500 hover:text-gray-700">← Downloads</Link>
                <h2 class="text-xl font-bold">Edit Download</h2>
            </div>
            <form @submit.prevent="submit" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">File (current: {{ download.file_name }})</label>
                    <input type="file" @change="e => form.file = e.target.files[0]" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"/>
                </div>
                ...
                <div class="flex gap-3">
                    <button type="submit" :disabled="form.processing" class="bg-blue-800 text-white px-6 py-2 rounded-lg text-sm hover:bg-blue-900 disabled:opacity-50">
                        {{ form.processing ? 'Saving...' : 'Update Download' }}</button>
